...
class Nginx < Formula
  ...
  def upload_install_args
    ...
    `wget -O /tmp/nginx_upload.tar.gz http://www.grid.net.ru/nginx/download/nginx_upload_module-2.2.0.tar.gz`
  ...
  def upload_progress_install_args
    ...
    `wget -O /tmp/nginx_upload-progress.tar.gz https://github.com/masterzen/nginx-upload-progress-module/tarball/v0.8.4`
  ...
  def install
    ...
    if ARGV.include? '--with-gru'
      ...
      args << upload_install_args
      args << upload_progress_install_args
    else
      ...
      args << upload_progress_install_args if ARGV.include? '--with-upload'
    end
